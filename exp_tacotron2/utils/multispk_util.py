...
def get_data_loaders(data_config, collate_fn):
    ...
    for phase in ["train", "dev"]:
        ...
        spk_id_paths = [ "data/fine-spk.npy" for utt_id in utt_ids] # <- spk_idを自身で実装するため、その処理の変更を学習処理で行う
...
def setup(config, device, collate_fn):
    ...
    # 学習率スケジューラ
    scheduler_name = config.train.optim.lr_scheduler.name
    ...
    # culculate the epoch by nepochs or max_train_steps from config
