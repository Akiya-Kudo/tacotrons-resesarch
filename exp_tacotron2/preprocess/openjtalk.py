...
# 正規表現を用いてフルコンテキストラベルから数値特徴量を抽出する関数
...
# フルコンテキストラベルから韻律記号付き音素列の抽出
def pp_symbols(labels, drop_unvoiced_vowels=True):
    """Extract phoneme(音素) + prosoody symbol( 韻律記号 ) sequence (系列) from input full-context labels
...
# 上記の定義で用意した辞書と照らし合わせて文字＋記号を数値に割り当てていく
